...
/**
 * Finds the maximum element in the tree rooted at 'root'.
 */
int max(Node* root) {
...
bool delete_node(Node*& root, KType key) {
    ...
    // case 2: target has two children
        ...
        Node* max = findMax(target->left);
		target->key = max->key;
		return delete_node(target->left, max->key);
    }
...
int height( Node* x ) {
	...
	int heightR = 0;
	if (hasChild(x)) {
		heightL += 1 + height(x->left);
		heightR += 1 + height(x->right);
	}
