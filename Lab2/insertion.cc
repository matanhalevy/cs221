...
void getArgs( int argc, char **argv ) {
  x_size = argc - 1;
  for(int ii = 0; ii < x_size; ii++ ){ // brackets
    x[ii] = atoi( argv[ ii + 1 ] );
  }
}

void scootOver( int jj ) {
  for(int kk = y_size; kk > jj; kk-- ){  // situation where y-1 = 0, and then y[kk -1] = -1?
    y[kk] = y[ kk - 1 ];                     // scoot first and insert? reverse
  } 
}
