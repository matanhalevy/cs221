...
void LinearHashDict::add(PuzzleState *key, PuzzleState *pred) {
  ...
  // TODO:  Your code goes here...
  float load = (float) number / (float) size;
  if (load >= 0.5)
    rehash();
  int probe = hash(key -> getUniqId());
  int i = 0;
  while(table[probe].key != NULL && table[probe].key != key){
    probe = (probe + i) & size;
    i++;
  }
  table[(probe+i) % size].key = key;
  table[(probe+i) % size].keyID = key->getUniqId();
  table[(probe+i) % size].data = pred;
  number++;
}
